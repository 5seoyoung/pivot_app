...
// MARK: - Toss-style Face Emoji (custom drawn)
...
struct PainFaceView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 6) {
                HStack(spacing: 12) {
                    if hasTearyEyes {
                        eyeWithTear(color: faceColor)
                        eyeWithTear(color: faceColor)
                    } else {
                        eye(score: score, color: faceColor)
                        eye(score: score, color: faceColor)
                    }
                }
    ...
    @ViewBuilder
    func eye(score: Int, color: Color) -> some View {
        if score >= 7 {
            ZStack {
                Text("×").font(.system(size: 11, weight: .black)).foregroundColor(color)
            }
        } else {
    ...
    func eyeWithTear(color: Color) -> some View {
        VStack(spacing: 1) {
            Circle().fill(color).frame(width: 7, height: 7)
            Capsule().fill(color.opacity(0.5)).frame(width: 3, height: 6)
        }
    }
}
    ...
    var body: some View {
        ZStack {
            RoundedRectangle(cornerRadius: size * 0.26)
                .fill(LinearGradient(
                    colors: [bgColor, bgColor.opacity(0.6)],
    ...
    var body: some View {
        TabView(selection: $selectedTab) {
            HomeView(selectedTab: $selectedTab)
                .tabItem { Label("홈", systemImage: selectedTab == 0 ? "house.fill" : "house") }
                .tag(0)
            ROMCameraView()
                .tabItem { Label("운동", systemImage: "camera.viewfinder") }
                .tag(1)
            MyRecordsView()
                .tabItem { Label("기록", systemImage: "chart.line.uptrend.xyaxis") }
                .tag(2)
            ProfileView()
                .tabItem { Label("내 정보", systemImage: selectedTab == 3 ? "person.fill" : "person") }
                .tag(3)
        }
        .tint(.brand)
    }
...
struct HomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    HomeQuickMenu(showPainCheck: $showPainCheck, showExercise: $showExercise, selectedTab: $selectedTab)
                        .padding(.bottom, 24)
                    if let pain = lastPain {
                        HomeRecentPainCard(record: pain).padding(.bottom, 24)
                    }
                    HomeWeeklyChart()
        ...
        .sheet(isPresented: $showPainCheck) {
            PainCheckView().presentationDetents([.large])
        }
        .sheet(isPresented: $showExercise) {
            ExerciseRecommendView().presentationDetents([.large])
        }
    }
}
...
struct HomeROMCard: View {
    ...
    let goal: Double = 120
    var progress: Double { min(flexion / goal, 1.0) }
    ...
    var body: some View {
        ...
        .background(LinearGradient(
            colors: [Color(hex: "EEF1FF"), Color(hex: "F5F0FF")],
            startPoint: .topLeading, endPoint: .bottomTrailing
        ))
        .clipShape(RoundedRectangle(cornerRadius: 20))
    ...
    var statsRow: some View {
        HStack(spacing: 0) {
            ROMStatView(value: flexion, label: "굴곡 (오늘)", color: .brand)
            Divider().frame(height: 44).padding(.horizontal, 20)
            ROMStatView(value: extension_, label: "신전 (오늘)", color: .success)
            Divider().frame(height: 44).padding(.horizontal, 20)
            ROMGoalView(goal: goal, current: flexion)
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack(alignment: .lastTextBaseline, spacing: 2) {
                Text(String(format: "%.0f", value)).font(.system(size: 28, weight: .bold)).foregroundColor(.textPrimary)
                Text("°").font(.system(size: 14)).foregroundColor(.textSecondary)
            }
...
struct ROMGoalView: View {
    let goal: Double
    let current: Double
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("\(Int(goal))°").font(.system(size: 28, weight: .bold)).foregroundColor(.textPrimary)
            Text("+\(Int(goal - current))° 남음").font(.system(size: 11, weight: .semibold)).foregroundColor(.success)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                QuickMenuCard(icon: "cross.case.fill", color: .warning, bgColor: .warningBg,
                              title: "통증 체크", subtitle: "오늘 이후 어때요?",
                ...
                QuickMenuCard(icon: "figure.run", color: .exMain, bgColor: .exLight,
                              title: "운동 추천", subtitle: "중기 회복기 맞춤",
                ...
                QuickMenuCard(icon: "calendar.badge.clock", color: .brand, bgColor: .brandBg,
                              title: "다음 진료", subtitle: "5월 26일 (월)",
                              badge: "D-26", badgeColor: .brand, badgeBg: .brandBg) {}
            }
            .padding(.horizontal, 20)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(alignment: .bottom, spacing: 6) {
                    ForEach(data, id: \.0) { item in
                        WeeklyBarItem(day: item.0, value: item.1, isToday: item.0 == "일")
                    }
                }
                .frame(height: 90)
                HStack {
    ...
    var body: some View {
        VStack(spacing: 5) {
            ...
            RoundedRectangle(cornerRadius: 5)
                .fill(isToday ? Color.brand : Color.brand.opacity(0.25))
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 10) {
                ...
                Text(badge).font(.system(size: 11, weight: .bold)).foregroundColor(badgeColor)
                    ...
                    .background(badgeBg).clipShape(Capsule())
            }
    ...
    var angleOverlay: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text("오늘 목표").font(.caption).foregroundColor(.white.opacity(0.55))
                Text("90°").font(.system(size: 26, weight: .bold)).foregroundColor(.white)
            }
        }
        ...
        VStack(spacing: 12) {
            ...
            Button {
                ...
                HStack(spacing: 10) {
                    ...
                    Text(isMeasuring ? "측정 완료" : "측정 시작").font(.system(size: 18, weight: .bold))
                }
        ...
        VStack(spacing: 3) {
            ...
            HStack(alignment: .lastTextBaseline, spacing: 1) {
                Text(String(format: "%.1f", value))
                    .font(.system(size: 26, weight: .bold)).foregroundColor(color)
    ...
    var body: some View {
        Button { toggleSelection() } label: {
            VStack(spacing: 8) {
                ZStack {
                    Circle()
                        .fill(selected ? Color.white.opacity(0.25) : iconColor.opacity(0.12))
            ...
            .overlay(RoundedRectangle(cornerRadius: 14)
                .stroke(selected ? Color.danger : Color.divider, lineWidth: 1.5))
...
struct NRSSliderSection: View {
    ...
    var nrsLabel: String {
        let arr = ["없음","매우 약함","약함","약함","보통","보통","심함","심함","매우 심함","극심함","극심함"]
        return arr[min(Int(nrsScore), 10)]
    }
        ...
        PivotCard {
            VStack(alignment: .leading, spacing: 16) {
                ...
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                    ForEach(painOptions, id: \.0) { item in
                        PainTypeButton(label: item.0, sfSymbol: item.1, iconColor: item.2,
...
struct ExercisePhaseButton: View {
    ...
    var body: some View {
        Button { withAnimation(.spring(response: 0.3)) { selectedPhase = phase } } label: {
            Text(phase.rawValue).font(.system(size: 15, weight: .medium))
                .foregroundColor(selected ? .white : .textSecondary)
                .padding(.horizontal, 18).padding(.vertical, 9)
                .background(selected ? Color.exMain : Color.surfaceBg)
                .clipShape(Capsule())
...
struct ExerciseCard: View {
    ...
    var exerciseRow: some View {
        HStack(spacing: 14) {
            PivotIcon(systemName: "figure.strengthtraining.functional", color: .exMain, bgColor: .exLight, size: 60)
            VStack(alignment: .leading, spacing: 5) {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(exercise.phase.rawValue).font(.caption.bold()).foregroundColor(.exMain)
                        ...
                        .background(Color.exLight).clipShape(Capsule())
                }
            }
            Spacer()
    ...
    var expandedContent: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 6) {
                ...
                Text("운동 시작").font(.system(size: 15, weight: .bold))
            }
            .foregroundColor(.white).padding(.horizontal, 20).padding(.vertical, 11)
    ...
    var summaryRow: some View {
        HStack(spacing: 10) {
            RecordSummaryCard(icon: "figure.walk.motion", color: .brand, bgColor: .brandBg,
                              value: "89°", label: "최대 굴곡", sub: "목표 90°")
    ...
    var body: some View {
        Button { withAnimation(.spring(response: 0.3)) { selectedRange = range } } label: {
            Text(range.rawValue).font(.system(size: 15, weight: .medium))
                .foregroundColor(selected ? .white : .textSecondary)
                .padding(.horizontal, 18).padding(.vertical, 9)
                .background(selected ? Color.brand : Color.surfaceBg)
                .clipShape(Capsule())
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            Text(label).font(.system(size: 11))
                .foregroundColor(stepState == .current ? color : .textSecondary)
    ...
    var profileCard: some View {
        PivotCard {
            HStack(spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text(profile?.patientCode ?? "환자 코드 미설정")
    ...
    var infoSection: some View {
        PivotCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Divider()
                InfoRow(icon: "figure.walk.motion", color: .success, title: "현재 단계",
                        value: profile?.phase ?? "중기")
                Divider()
                InfoRow(icon: "target", color: .warning, title: "굴곡 목표", value: "120°")
            }
        }
    }
